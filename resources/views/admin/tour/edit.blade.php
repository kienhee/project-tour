...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <x-alert />
                <x-header-table tableName="Cập nhật {{ $moduleName }}" link="dashboard.tour.index"
                    linkName="Danh sách {{ $moduleName }}" />
                <div class="card-body">
                    <form action="{{ route('dashboard.tour.update', $tour->id) }}" method="POST"
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-12">
                                <div class="upload__box d-flex justify-content-center flex-column align-items-center gap-3">
                                    <img id="img_preview" class="img-fluid object-fit-contain"
                                        src="{{ $tour->cover ?? asset('images/pngtree-image-upload-icon-photo.png') }}"
                                        alt="your image" />
                                    ...
                                    <input type="text" name="cover" value="{{ $tour->cover }}" hidden>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('title') is-invalid @enderror " type="text"
                                    ...
                                    placeholder="VD: Du lịch chùa A - Đền B - Biển C, ..v.v" />
                                @error('title')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('starting_point') is-invalid @enderror" type="text"
                                    ...
                                    value="{{ old('starting_point') ?? $tour->starting_point }}"
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('road_map') is-invalid @enderror" type="text"
                                    id="road_map" name="road_map" value="{{ old('road_map') ?? $tour->road_map }}"
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('date_of_department') is-invalid @enderror"
                                    ...
                                    value="{{ old('date_of_department') ?? $tour->date_of_department }}" />
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('return_date') is-invalid @enderror" type="datetime-local"
                                    ...
                                    value="{{ old('return_date') ?? $tour->return_date }}" />
                                @error('return_date')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('price_large') is-invalid @enderror" type="text"
                                    id="price_large" name="price_large"
                                    value="{{ old('price_large') ?? $tour->price_large }}"
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('price_small') is-invalid @enderror" type="text"
                                    id="price_small" name="price_small"
                                    value="{{ old('price_small') ?? $tour->price_small }}" placeholder="Giá vé trẻ em" />
                                @error('price_small')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('amount_of_people') is-invalid @enderror" type="text"
                                    id="amount_of_people" name="amount_of_people"
                                    value="{{ old('amount_of_people') ?? $tour->amount_of_people }}"
                                ...
                                <select class="form-select @error('vehicle_id') is-invalid @enderror" name="vehicle_id"
                                    ...
                                    @if (getAllVehicle()->count() > 0)
                                        @foreach (getAllVehicle() as $item)
                                            <option
                                                {{ old('vehicle_id') == $item->id || $tour->vehicle_id == $item->id ? 'selected' : '' }}
                                                value="{{ $item->id }}">
                                                {{ $item->name }}</option>
                                        @endforeach
                                    @endif
                                ...
                                <select class="form-select @error('destination_id') is-invalid @enderror"
                                    ...
                                    @if (getAllVehicle()->count() > 0)
                                        @foreach (getAllDestination() as $item)
                                            <option
                                                {{ old('destination_id') == $item->id || $tour->destination_id == $item->id ? 'selected' : '' }}
                                                value="{{ $item->id }}">
                                                {{ $item->name }}</option>
                                        @endforeach
                                    @endif
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea class="form-control @error('content') is-invalid @enderror " id="content-product" rows="3"
                                    name="content">{{ old('content') ?? $tour->content }}</textarea>
                                @error('content')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
