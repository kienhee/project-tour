...
@section('content')
<x-breadcrumb parentName="{{ $moduleName }}" parentLink="dashboard.group.index"
        ...
        <form action="{{route('dashboard.group.postPermissions',$group->id)}}" method="POST" class="pb-2">
            ...
            <table class="table">
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    <tr>
                        <td><i class="fab fa-angular fa-lg text-danger "></i> <strong>{{$module->title}}</strong></td>
                        ...
                        <td>
                            <div class="form-check">
                                ...
                                <input class="form-check-input" type="checkbox" name="role[{{$module->routeName}}][]"
                                    @if (isRole($roleData,$module->routeName,$roleName))
                                @checked(true)
                                @endif
                                value="{{$roleName}}" id="role_{{$module->routeName}}_{{$roleName}}">
                            </div>
                        </td>
                        @endforeach
