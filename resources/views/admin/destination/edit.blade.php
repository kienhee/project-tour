...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <x-alert />
                <x-header-table tableName="Cập nhật {{ $moduleName }}" link="dashboard.destination.index"
                    linkName="Danh sách {{ $moduleName }}" />
                <!-- Account -->
                <div class="card-body">
                    <form id="formAccountSettings" action="{{ route('dashboard.destination.update', $destination->id) }}"
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-12">
                                <div class="upload__box d-flex justify-content-center flex-column align-items-center gap-3">
                                    <img id="img_preview" class="img-fluid object-fit-contain"
                                        src="{{ $destination->cover ?? asset('images/pngtree-image-upload-icon-photo.png') }}"
                                        alt="your image" />
                                    ...
                                    <input type="text" name="cover" value="{{ $destination->cover }}" hidden>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror " type="text"
                                    ...
                                    value="{{ old('name') ?? $destination->name }}" placeholder="Tên địa điểm" autofocus />
                                @error('name')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
